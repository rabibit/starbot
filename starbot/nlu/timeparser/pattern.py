...
patterns = re.compile(
    r'''(([前昨今明后])([天日])?([早晚])([晨上间])?)
        ...
        |(\d+(分钟|min))
        |([13]刻钟)
        |(([上这本下])+(周|星期)([一二三四五六七天日]|[0-6])?)
        |((周|星期)([一二三四五六七天日]|[0-6]))
        |(([早晚])?([0-2]?[0-9]([点时])半)(am|AM|pm|PM)?)
        |(([早晚])?(\d+[:：]\d+([:：]\d+)*)\s*(am|AM|pm|PM)?)
        |(([早晚])?([0-2]?[0-9]([点时])[13一三]刻)(am|AM|pm|PM)?)
        |(([早晚])?(\d+[时点](\d+)?分?(\d+秒?)?)\s*(am|AM|pm|PM)?)
        |(大+([前后])天)
        |(([零一二三四五六七八九十百千万]+|\d+)世)
        ...
        |((一|二|两|三|四|五|六|七|八|九|十|百|千|万|几|多|上|\d+)+个?([天日周月年])([后前半]))
        |((胜利的)日子)
        |(青春期)
